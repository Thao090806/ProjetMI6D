...
Personnage creer_personnage(char *nom, char *description, int pv_max, int attaque, int defense, int agilite, int vitesse) {
    if (nom == NULL || description == NULL) {
        fprintf(stderr, "Erreur : nom ou description est NULL.\n");
        exit(1);
    }
    if (pv_max <= 0 || attaque < 0 || defense < 0 || agilite < 0 || vitesse < 0) {
        fprintf(stderr, "Erreur : Valeurs de stats invalides.\n");
        exit(1);
    }
...
Competence creer_competence(char *nom, char *description, int coefficient, int tours_actifs, int tours_recharge) {
    ...
    if (coefficient <= 0 || tours_actifs < 0 || tours_recharge < 0) {
        fprintf(stderr, "Erreur : Valeurs de competence invalides.\n");
        exit(1);
    }
...
void charger_personnages(Personnage personnages[]) {
    if(personnages == NULL) {
        fprintf(stderr, "Erreur : tableau de personnages est NULL.\n");
        exit(1);
    }
